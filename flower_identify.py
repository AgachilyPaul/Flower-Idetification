...
INPUT_DATA = train.INPUT_DATA
BOTTLENECK_TENSOR_NAME = data_process.BOTTLENECK_TENSOR_NAME
JPEG_DATA_TENSOR_NAME = data_process.JPEG_DATA_TENSOR_NAME
# 测试数据
file_path = '/Users/lyx/Documents/PycharmProjects/Flowers/flower_photos/sunflowers/6953297_8576bf4ea3.jpg'
    ...
    with tf.Graph().as_default() as graph:
        with tf.Session().as_default() as sess:
            with tf.gfile.FastGFile(TRAIN_FILE, 'rb') as f:
                graph_def = tf.GraphDef()
                graph_def.ParseFromString(f.read())
                final_tensor, bottleneck_input = tf.import_graph_def(graph_def, name='',
